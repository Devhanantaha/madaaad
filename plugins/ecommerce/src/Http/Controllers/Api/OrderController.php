...
use Plugin\Ecommerce\Http\Requests\CustomerCheckoutRequest;
...
class OrderController extends Controller
{
    ...
    public function applyCoupon(Request $request)
    {
        ...
        try {
            ...
            if ($coupon_details->maximum_spend_mount != null && $coupon_details->maximum_spend_mount != '0' && $coupon_details->maximum_spend_mount < $total_cart_price) {
     ...
     */
    public function createCustomerOrder(CustomerCheckoutRequest $request)
    {
        $response = $this->order_repository->customerCheckout($request);
    ...
    {
        $response = $this->order_repository->guestCheckout($request);
        return response()->json($response);
    }

    public function paymentQueueVerify(Request $request)
    {
        $pi = $request['pi'];
        $payment_info = $request['payment_info'] ?? null;
        return (new \Plugin\Ecommerce\Http\Controllers\Payment\PaymentController)->payment_success($pi, $payment_info);
    }
    /**
     * Will cancel a order
     * 
    ...
    {
        $link = $this->order_repository->makeOrderPaymentLink($request['order_id'], $request['origin']);
        if ($link != null) {
            return response()->json(
                [
                    'success' => true,
    ...
    public function customerOrderDetails(Request $request)
    {
        if ($request->has('order_id')) {
            $order_details = $this->order_repository->customerOrderDetails(auth('jwt-customer')->user()->id, $request['order_id']);
            ...
            return new SingleOrderCollection($order_details);
        } else {
    ...
    {
        $order_details = $this->order_repository->OrderDetailsByOrderCode($request->order_code);
        if ($order_details == null) {
