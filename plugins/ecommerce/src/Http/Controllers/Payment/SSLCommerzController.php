...
class SSLCommerzController extends Controller
{
    ...
    public function index($payment_queue)
    {
        ...
        $post_data['value_a'] = $payment_queue->uid;
        $post_data['success_url'] = route('sslcommerz.success.payment', ['pi' => $pi]);
        $post_data['fail_url'] = route('sslcommerz.fail.payment', ['pi' => $pi]);
        $post_data['cancel_url'] = route('sslcommerz.cancel.payment', ['pi' => $pi]);
    ...
    public function success($pi, Request $request)
    {
        try {
            if ($request->status === 'VALID') {
                return (new PaymentController)->payment_success($request->value_a, 'transaction id ' . $request->tran_id);
            } else {
