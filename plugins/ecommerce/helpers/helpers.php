...
if (!function_exists('currencyExchange')) {
    ...
    function currencyExchange($value, $formatting = true, $target_currency_id = NULL, $is_html = true)
    {
        ...
        //Convert and format to target currency
        if ($target_currency_id != null) {
            $target_currency = Plugin\Ecommerce\Models\Currency::where('id', $target_currency_id)->first();
            ...
            if ($formatting && $is_html) {
                $formatting_value = number_format($value, $target_currency->number_of_decimal, $target_currency->decimal_separator, $target_currency->thousand_separator);
                $position = $target_currency->position;

                switch ($position) {
                    case "1":
                        return "<span class='currency-font'>" . $target_currency->symbol . '' . $formatting_value . "</span>";
                        break;
                    case "2":
                        return "<span class='currency-font'>" . $formatting_value . '' . $target_currency->symbol . "</span>";
                        break;
                    case "3":
                        return "<span class='currency-font'>" . $target_currency->symbol . ' ' . $formatting_value . "</span>";
                        break;
            ...
            if ($formatting && !$is_html) {
                ...
                switch ($position) {
                    case "1":
                        return  $target_currency->symbol . '' . $formatting_value;
                        break;
                    case "2":
                        return  $formatting_value . '' . $target_currency->symbol;
                        break;
                    case "3":
                        return  $target_currency->symbol . ' ' . $formatting_value;
                        break;
...
if (!function_exists('convertToDefaultCurrency')) {
     ...
     * 
     */
    function convertToDefaultCurrency($value, $from_currency_id = NULL, $formatting = false, $is_html = false)
    {
        ...
        if ($from_currency_id != null) {
            $from_currency = Plugin\Ecommerce\Models\Currency::where('id', $from_currency_id)->first();
        }

        $converted_amount = ($value * $default_currency_details->conversion_rate) / $from_currency->conversion_rate;
        $value = $converted_amount;
        //Format default currency
        if ($default_currency_details != null) {
            if (!$formatting) {
