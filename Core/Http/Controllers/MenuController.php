...
class MenuController extends Controller
{
    ...
    try {
      ...
      if (isset(request()->menu_group_id)) {
        array_push($match_case, [
          'tl_menus.menu_group_id',
          '=',
          request()->menu_group_id
        ]);
      ...
      for ($i = 0; $i < sizeof($data); $i++) {
        ...
        if ($data[$i]->icon == null || $data[$i]->icon == 'null' || $data[$i]->icon == '' ||  $data[$i]->icon == ' ') {
          ...
          if (isset(request()->lang_id) && request()->lang_id != defaultLanguage()) {
  ...
  public function updateTreeViewData(Request $request)
  {
    try {
      ...
      if (isset($request['name']) && $request['name'] != "" && isset($request['lang_id']) && $request['lang_id'] == defaultLanguage()) {
