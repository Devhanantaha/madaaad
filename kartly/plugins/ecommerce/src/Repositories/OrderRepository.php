...
use Plugin\Ecommerce\Models\LocationWiseShippingRate;
...
use Plugin\Ecommerce\Models\LocationWiseShippingRateCountry;
...
class OrderRepository
{
    ...
    public function checkoutConfiguration()
    {
        try {
            $data = [
                ...
                'enable_minimum_order_amount'             => SettingsRepository::getEcommerceSetting('enable_minimum_order_amount'),
                ...
                'enable_pickup_point_in_checkout'          => SettingsRepository::getEcommerceSetting('enable_pickup_point_in_checkout'),
        ...
        try {
            ...
            return
                [
                    ...
                    'file_id' => $attachment,
                    'path' => asset(getFilePath($attachment))
                ];
        } catch (\Exception $e) {
            return null;
        }
    ...
    {
        $query = Orders::where('customer_id', $customer_id)->orderBy('id', 'DESC');
        if ($request->has('search_key') && $request['search_key'] != null) {
            $query = $query->where('order_code', 'like', '%' . $request['search_key'] . '%');
        }
    ...
    public function shippingOptions($request)
    {
        try {
            ...
            if ($request->has('shipping_type')  && $request['shipping_type'] == 'home_delivery') {
                ...
                if ($shipping_option != null && $shipping_option == config('ecommerce.shipping_cost_options.location_wise_rate')) {
    ...
    public function shippingProfileBasedShippingCostCalculation($request, $allow_free_shipping = true)
    {
            ...
            if (sizeof($all_product_shipping_profiles) > 0) {
                ...
                foreach ($all_product_shipping_profiles as $key => $profile) {
                    ...
                    if (sizeof($validate_shipping_rates) > 0) {
                        ...
                        array_push($not_available_products, $profile['product']);
                    }
                }
        ...
        if (count($shipping_zone_ids) < 1) {
            return [
                'success' => true,
                'shipping_available' => false,
                'not_available_products' => $product_list,
            ];
        }
    }
    ...
    public function customerCheckout($request)
    {
        try {
            ...
            $order->pickup_point_id = $request->has('pickup_point') && $request['pickup_point'] != 'null' ? $request['pickup_point'] : NULL;
            ...
            if ($request['wallet_payment'] == config('settings.general_status.active') && isActivePlugin('wallet')) {
                ...
                $wallet_transaction->recharge_type = config('ecommerce.wallet_recharge_type.checkout');
            ...
            DB::commit();
            $redirect_url = $this->generateOrderPaymentLink($order->id, auth('jwt-customer')->user()->id, 'registered', $request['payment_id'], $total_payable_amount, $request['wallet_payment'], $request['origin']);
            return [
                'success' => true,
                'order_id' => $order->id,
                'redirect_url' => $redirect_url,
                'config' => $this->paymentMethodConfig($order->id)
            ];
        } catch (\Exception $e) {
            DB::rollBack();
            return [
    ...
    public function guestCheckout($request)
    {
        try {
            ...
            if ($request->has('create_new_account')) {
                ...
                $customer->name = $request->has('pickup_point') ? $request['customer_name'] : $shipping_address['name'];
                $customer->email = $request->has('pickup_point') ? $request['customer_email'] : $shipping_address['email'];
                ...
                $guest_customer->name = $request->has('pickup_point') ? $request['customer_name'] : $shipping_address['name'];
                $guest_customer->email = $request->has('pickup_point') ? $request['customer_email'] : $shipping_address['email'];
            ...
            if ($request->has('billing_address')) {
                ...
                $customer_address->customer_id = $customer_id;
                $customer_address->guest_customer = $guest_customer_id;
                $customer_address->name = $billing_address['name'];
                $customer_address->country_id = $billing_address['country_id'];
                $customer_address->state_id = $billing_address['state_id'];
                $customer_address->city_id = $billing_address['city_id'];
                $customer_address->postal_code = $billing_address['postal_code'];
                $customer_address->address = $billing_address['address'];
                $customer_address->phone = $billing_address['phone'];
                $customer_address->save();
            ...
            $redirect_url = null;
            $redirect_url = $this->generateOrderPaymentLink($order->id, $guest_customer_id, 'guest', $request['payment_id'], $total_payable_amount, $request['wallet_payment'], $request['origin']);
    ...
    public static function generateOrderCode()
    {
        ...
        $prefix_separator = SettingsRepository::getEcommerceSetting('order_code_prefix_separator');
    ...
    {
        if ($request['coupon_discounts'] == null) {
            return 0;
        }
    ...
    public function deleteOrder($order_id)
    {
        try {
            ...
            OrderHasProducts::where('order_id', $order_id)->delete();
            Orders::where('id', $order_id)->delete();
            GuestCustomers::where('order_id', $order_id)->delete();
    ...
    public function getShippingLabelContent($order_id, $order_products)
    {
        try {
            ...
            $system_properties =
                [
                    'title' => get_setting('system_name'),
                    'logo' => SettingsRepository::getEcommerceSetting('invoice_logo') != null ? getFilePath(SettingsRepository::getEcommerceSetting('invoice_logo'), false) : getFilePath(get_setting('white_background_logo'), false),
     ...
     * @return String
     */
    public function makeOrderPaymentLink($order_id, $origin = "web")
    {
        try {
            $order = Orders::find($order_id);
            if ($order != null) {
                ...
                $link = $this->generateOrderPaymentLink($order->id, $order->customer_id, $customer_type, $order->payment_method, $order->total_payable_amount, $order->wallet_payment, $origin);
                return $link;
            } else {
                return null;
            }
     ...
     */
    public  function generateOrderPaymentLink($order_id, $customer_id, $customer_type, $payment_method_id, $payable_amount, $is_wallet_payment = 2, $origin = "web")
    {

        try {
            $base_url = url('/');
            if (
                $is_wallet_payment == config('settings.general_status.active') ||
                $payment_method_id == config('ecommerce.payment_methods.cod') ||
                $payment_method_id == config('ecommerce.payment_methods.bank')
            ) {
                if ($origin == "app") {
                    return "none";
                }
            ...
            $payment_method = PaymentMethods::find($payment_method_id);

            if ($payment_method == null) {
                return null;
            }

            $payment_queue = PaymentQueue::where('order_id', $order_id)->first();

            if ($payment_queue == null) {
                $payment_queue = new PaymentQueue();
            }
            $payment_queue->amount = $payable_amount;
            $payment_queue->currency_id = NULL;
            $payment_queue->payment_method_id = $payment_method?->id;
            $payment_queue->type = "checkout";
            $payment_queue->return_url = $base_url;
            $payment_queue->origin = $origin;
            $payment_queue->order_id = $order_id;
            if ($customer_type == 'guest') {
                $payment_queue->guest_customer_id = $customer_id;
            } else {
                $payment_queue->customer_id = $customer_id;
            }
            $payment_queue->save();
    ...
    public function paymentMethodConfig($order_id)
    {
        try {
            $payment_queue = PaymentQueue::where('order_id', $order_id)->first();
            if ($payment_queue == null) {
            ...
            $config = (new \Plugin\Ecommerce\Http\Controllers\Payment\PaymentController)->getPaymentMethodConfig($payment_queue->uid);
            return $config;
        } catch (\Exception $e) {
            return null;
        }
