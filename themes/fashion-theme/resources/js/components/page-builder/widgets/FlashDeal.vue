<template>
  <section class="deals-section home-page-section">
    <div class="row align-items-center my-3">
      ...
      <div class="col-md-6 d-flex  flex-wrap justify-content-md-end align-items-center gap-4">
        <countdown class="justify-content-md-end" :deadline="dealDetails.deadline" />
        <router-link :to="`/deals/${dealDetails.permalink}`" class="btn">{{ $t("View All") }}</router-link>
      </div>
    </div>
    <swiper v-if="dealProducts.length && properties.style == 'slider'" :slidesPerView="slider_items.desktop"
      :modules="modules" :spaceBetween="24" :grid="{
        rows: row_count,
        fill: 'row',
      }" :autoplay="{
        delay: 2500,
